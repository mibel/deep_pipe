...
class WhiteMatterHyperintensity(Dataset):
    ...
    def load_mscan(self, patient_id):
            ...
            res = []
            for modalities in ['pre/reg_FLAIR.nii.gz', 'pre/reg_T1.nii.gz']: # reg_t1.nii.gz
                image = os.path.join(path_to_modalities, modalities)
                x = nib.load(image).get_data().astype('float32')

                if modalities == 'pre/reg_FLAIR.nii.gz':
                    brain = path_to_modalities + '/pre/brainmask_reg_T1_mask.nii.gz'
    ...
    def load_segm(self, patient_id):
        path_to_modalities = self.idx_to_path[patient_id]
        x = nib.load(os.path.join(path_to_modalities, 'reg_wmh.nii.gz')).get_data() # 'wmh_reg.nii.gz'
        x = self._reshape_to(x, new_shape=self.spatial_size)
        x[x < 0.5] = 0
